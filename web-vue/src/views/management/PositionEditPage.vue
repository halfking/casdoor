<template>
  <div class="position-edit-container">
    ...
    >
      ...
      <a-form-item :label="$t('position.Org Tree Code')" name="orgTreeCode">
        <a-input v-model:value="form.orgTreeCode" placeholder="关联的组织树节点编码" />
      </a-form-item>

      <a-form-item :label="$t('position.Agent Provider')" name="agentProvider">
        <a-input v-model:value="form.agentProvider" placeholder="AI智能体Provider名称" />
      </a-form-item>

      <a-form-item :label="$t('position.Agent Model')" name="agentModel">
        <a-input v-model:value="form.agentModel" placeholder="AI模型名称" />
      </a-form-item>
...
const fetchPosition = async (id: string) => {
  ...
  try {
    ...
    if (res.status === "ok" && res.data) {
      const data = res.data;
      Object.assign(form, {
        roleOwner: data.roleOwner || "built-in",
