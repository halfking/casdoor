          ...
          >
            <template #bodyCell="{ column, record, index }">
              ...
              <template v-else-if="column.key === 'role'">
                ...
                >
                  <a-select-option value="role-built-in-readonly">Built-in Readonly</a-select-option>
                  <a-select-option value="role-built-in-admin">Built-in Admin</a-select-option>
                </a-select>
              </template>
...
async function fetchWorkflow() {
  ...
  try {
    const res = await WorkflowApi.getWorkflow(String(workflowId));
    if (res.status === "ok" && res.data) {
      formState.id = res.data.id;
      formState.name = res.data.name;
      formState.displayName = res.data.displayName || "";
      formState.department = res.data.department;
      formState.description = res.data.description || "";
      formState.steps = parseSteps(res.data.steps);
    }
  } catch (error) {
...
async function handleSave() {
  ...
  try {
    ...
    let res;
    if (isEdit.value) {
      res = await WorkflowApi.updateWorkflow(payload);
    } else {
