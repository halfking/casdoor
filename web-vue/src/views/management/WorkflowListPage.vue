<template>
  <div class="workflow-list-container">
    ...
    <a-table
      ...
      :row-key="(record: Workflow) => `${record.owner}/${record.name}`"
      @change="handleTableChange"
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'displayName'">
          <router-link :to="`/management/workflows/${record.owner}/${record.name}`">
...
function handleEdit(record) {
  router.push(`/management/workflows/${record.owner}/${record.name}`);
}
...
async function handleDelete(record) {
  try {
    const res = await WorkflowApi.deleteWorkflow({ owner: record.owner, name: record.name });
    if (res.status === 'ok') {
