// OAuth Provider configuration & auth URL generation
// Ported from web/src/auth/Provider.js
...
export function clearCodeVerifier(state: string) {
...
export const authInfo: Record<string, ProviderAuthInfo> = {
  ...
  WeChat: {
    scope: "snsapi_login", endpoint: "https://open.weixin.qq.com/connect/qrconnect",
    mpScope: "snsapi_userinfo", mpEndpoint: "https://open.weixin.qq.com/connect/oauth2/authorize",
  },
  WeChatMiniProgram: { scope: "", endpoint: "https://mp.weixin.qq.com/" },
  ...
  WeCom: {
    scope: "snsapi_userinfo", endpoint: "https://login.work.weixin.qq.com/wwlogin/sso/login",
  ...
  Lark: { scope: "", endpoint: "https://open.feishu.cn/open-apis/authen/v1/index", endpoint2: "https://accounts.larksuite.com/open-apis/authen/v1/authorize" },
  ...
  Infoflow: { scope: "", endpoint: "https://xpc.im.baidu.com/oauth2/authorize" },
  ...
  Steam: { scope: "", endpoint: "https://steamcommunity.com/openid/login" },
  ...
  Custom: { scope: "", endpoint: "https://example.com/" },
  Bilibili: { scope: "", endpoint: "https://passport.bilibili.com/register/pc_oauth2.html" },
...
export function getQueryParamsFromState(state: string): string {
...
/* ───────── OAuth URL parameters parsing ───────── */

function getRefinedValue(value: string | null): string {
  return value ?? "";
}

function getRawGetParameter(key: string): string {
...
export interface OAuthGetParams {
  clientId: string;
  responseType: string;
  redirectUri: string;
  scope: string;
...
export function getOAuthGetParameters(params?: URLSearchParams): OAuthGetParams | null {
  const queries = params ?? new URLSearchParams(window.location.search);
  ...
  const samlRequest = getRefinedValue(lowercaseQueries["samlrequest"]);
  const relayState = getRefinedValue(lowercaseQueries["relaystate"]);
  const noRedirect = getRefinedValue(lowercaseQueries["noredirect"]);
...
export function getCasParameters(params?: URLSearchParams): {
  ...
  return {
    ...
    renew: getRefinedValue(queries.get("renew")),
    gateway: getRefinedValue(queries.get("gateway")),
  };
}
...
export function getCasLoginParameters(owner: string, name: string): {
  ...
  if (service === "") {
    service = getRefinedValue(new URLSearchParams(window.location.search).get("service"));
  }
  return {
    id: `${owner}/${encodeURIComponent(name)}`,
    service,
    type: "cas",
  };
}
...
export function getProviderUrl(provider: { category?: string; type?: string; domain?: string }): string {
  if (provider.category === "OAuth") {
    const type = provider.type?.startsWith("Custom") ? "Custom" : (provider.type || "");
    const info = authInfo[type];
    if (!info?.endpoint) return "";
    try {
      const urlObj = new URL(info.endpoint);
      let host = urlObj.host;
      const tokens = host.split(".");
      if (tokens.length > 2) {
        host = tokens.slice(1).join(".");
      }
...
export function getAuthUrl(
  application: ApplicationForAuth | null,
  provider: ProviderForAuth | null,
  method: string,
  code?: string
): string {
  if (!application || !provider) return "";

  const type = provider.type.startsWith("Custom") ? "Custom" : provider.type;
  let endpoint = authInfo[type]?.endpoint ?? "";
  const redirectOrigin = application.forcedRedirectOrigin || window.location.origin;
  ...
  const isShortState =
    (provider.type === "WeChat" && navigator.userAgent.includes("MicroMessenger")) ||
  ...
  const state = getStateFromQueryParams(applicationName, provider.name, method, isShortState);
  ...
  } else if (provider.type === "Nextcloud" && provider.domain) {
    endpoint = `${provider.domain}/apps/oauth2/authorize`;
  } else if (provider.type === "Lark" && provider.disableSsl) {
    endpoint = authInfo.Lark.endpoint2!;
  }

  // Standard OAuth providers (use common URL format)
  const standardProviders = new Set([
    "Google", "GitHub", "Facebook", "Weibo", "Gitee", "LinkedIn", "GitLab", "AzureAD",
    ...
    "Spotify", "Strava", "Stripe", "Tumblr", "Twitch", "Typetalk", "Uber", "VK", "Wepay",
    "Xero", "Yahoo", "Yammer", "Yandex", "Zoom",
  ]);

  if (standardProviders.has(provider.type)) {
  ...
  switch (provider.type) {
    ...
    case "WeChat":
      if (navigator.userAgent.includes("MicroMessenger")) {
        return `${authInfo.WeChat.mpEndpoint}?appid=${provider.clientId2}&redirect_uri=${redirectUri}&state=${state}&scope=${authInfo.WeChat.mpScope}&response_type=code#wechat_redirect`;
      }
    ...
    case "WeCom": {
      const wecomEndpoint = authInfo.WeCom;
      if (provider.subType === "Internal") {
        if (provider.method === "Silent") {
          return `${wecomEndpoint.silentEndpoint}?appid=${provider.clientId}&redirect_uri=${redirectUri}&state=${state}&scope=${scope}&response_type=code#wechat_redirect`;
        } else if (provider.method === "Normal") {
          return `${wecomEndpoint.internalEndpoint}?login_type=CorpApp&appid=${provider.clientId}&agentid=${provider.appId}&redirect_uri=${redirectUri}&state=${state}`;
        }
        ...
        if (provider.method === "Silent") {
          ...
          return `${wecomEndpoint.endpoint}?login_type=ServiceApp&appid=${provider.clientId}&redirect_uri=${redirectUri}&state=${state}`;
        }
        return `https://error:not-supported-provider-method:${provider.method}`;
      }
    ...
    case "Lark": {
      const larkRedirect = provider.disableSsl ? encodeURIComponent(redirectUri) : redirectUri;
      return `${endpoint}?app_id=${provider.clientId}&redirect_uri=${larkRedirect}&state=${state}`;
    }
    ...
    default:
      // Custom or unknown — treat as Custom with optional PKCE
      if (type === "Custom") {
