...
标准做法是把菜单、部门、岗位等平台扩展数据纳入统一 bootstrap 流程，由 `scripts/apply-platform-config.sh --with-platform-bootstrap` 执行。若需要编写自定义批量 SQL，至少应保持 `ON CONFLICT DO NOTHING` 的幂等约束，例如：
...
如果确实需要为某个新应用单独准备一次性 SQL，仍应保证幂等，并通过 PostgreSQL 执行：
...
- **菜单 (17项)**: 由平台 bootstrap 流程导入，包含 7 个顶级/容器菜单 + 10 个叶子菜单
- **部门 (3项)**: 技术部、产品部、运营部
