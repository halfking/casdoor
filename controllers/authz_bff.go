...
func shouldBypassBffPermissionCheck(subjectUser *object.User) bool {
	return subjectUser != nil && subjectUser.IsGlobalAdmin()
}

func shouldReturnAllTenantsForOwner(user *object.User, owner string) bool {
	return user != nil && user.IsGlobalAdmin() && owner == "built-in"
}
...
func (c *ApiController) ResolvePermissionsForBff() {
	...
	for _, check := range req.Checks {
		...
		allowed := shouldBypassBffPermissionCheck(subjectUser)
		if !allowed {
...
func (c *ApiController) GetTenantTreeForBff() {
	...
	if user.IsGlobalAdmin() {
		if shouldReturnAllTenantsForOwner(user, owner) {
			organizations, err = object.GetAllOrganizations()
		} else {
