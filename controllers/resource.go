...
func (c *ApiController) UploadResource() {
	...
	case "logo":
		_, applicationId := util.GetOwnerAndNameFromIdNoCheck(strings.TrimSuffix(fullFilePath, filepath.Ext(fullFilePath)))
		if applicationId == "" {
		...
		if applicationObj == nil {
			applicationObj, err = object.GetApplication(application)
			if err != nil {
				c.ResponseError(err.Error())
				return
			}
		...
		if applicationObj != nil {
			applicationObj.Logo = fileUrl
			_, err = object.UpdateApplication(applicationObj.GetId(), applicationObj, true, c.GetAcceptLanguage())
			if err != nil {
				c.ResponseError(err.Error())
				return
			}
