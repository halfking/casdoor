...
func (c *ApiController) Logout() {
	...
	if accessToken == "" && redirectUri == "" {
		...
		if err := c.logoutCurrentSession(user); err != nil {
			c.ResponseError(err.Error())
			return
		}
...
func (c *ApiController) logoutCurrentSession(user string) error {
	beegoSessionId := ""
	...
	if user != "" {
		if err := c.deleteUserSessionById(user, beegoSessionId); err != nil {
			return err
		}
	}
...
func (c *ApiController) deleteUserSession(user string) error {
	...
	return c.deleteUserSessionById(user, beegoSessionId)
}

func (c *ApiController) deleteUserSessionById(user string, beegoSessionId string) error {
	owner, username, err := util.GetOwnerAndNameFromIdWithError(user)
	if err != nil {
		return err
	}
