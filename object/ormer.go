...
func (a *Ormer) createTable() {
	...
	err = a.Engine.Sync2(new(Menu))
	if err != nil {
		panic(err)
	}

	err = a.Engine.Sync2(new(Department))
	if err != nil {
		panic(err)
	}

	err = a.Engine.Sync2(new(Post))
	if err != nil {
		panic(err)
	}

	err = a.Engine.Sync2(new(Position))
	if err != nil {
		panic(err)
	}

	err = a.Engine.Sync2(new(OrgTemplate))
	if err != nil {
		panic(err)
	}
